...
class App(Frame):
	...
	def MANDATORY(self):
		...
		self.file1_title = Label(self.frame, text="Bowtie2 Build", font="Times 20 bold", bg="white").grid(row = x, column = 0, columnspan = 10, padx = 5, pady = (5,5), sticky = "we")
		x += 1
		...
		self.fill_mandat = Label(self.frame, text="""Before running it is recommended to look at https://support.illumina.com/sequencing/sequencing_software/igenome.html
for reference files to download as creating reference files from scratch can be time intensive.""", font="Times 16", bg="white").grid(row = x, column = 0, columnspan = 10, padx = 5, pady = 5, sticky = "we")
		x += 1
		...
		self.label_filename = Label(self.frame, text="Input File: Pick input file to use as reference.", relief=FLAT, bg="white")
		...
		Label(self.frame, text="Output File: Will default to hg with file type appended to end", relief=FLAT, bg="white").grid(row=x,column=1, padx = 5, pady = 5, sticky = "w")
		x += 1

		##Run Button which goes to function: Check_Options when clicked
		self.run_button = Button(self.frame, text="Run Bowtie2 Build", command=self.Check_Options)
	...
	def Check_Options(self):
		...
		globstring = "./Bowtie2/bowtie2-2.3.4.1/bowtie2-build "
		...
		if(self.var_out_file.get() == "Output Filename" or self.var_out_file.get() == ""):
			globstring += "./Bowtie2/bowtie2-2.3.4.1/" + "hg"
		else:
			globstring += "./Bowtie2/bowtie2-2.3.4.1/" + self.var_out_file.get()
		...
		if(errors == ''):
			...
			if('bowtie2-build-l' in os.listdir('./Bowtie2/bowtie2-2.3.4.1/')):
			...
			elif('Makefile' in os.listdir('./Bowtie2/bowtie2-2.3.4.1/')):
				self.err_message.config(text="Compiling....", font="Times 18")
				self.update()
				subprocess.call(["make", "-C", "./Bowtie2/bowtie2-2.3.4.1/"])
				self.err_message.config(text="Running....", font="Times 18")
				self.update()
				p = subprocess.Popen(cmd_line, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
